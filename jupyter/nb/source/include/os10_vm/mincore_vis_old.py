...
def generate_mincore_frames(fig, mincore_dat, animation_speed):
...
def animate_mincore_x(mincore_dat, animation_speed):
    fig = plt.figure()
    ani = anm.FuncAnimation(fig, lambda x: x, repeat=0,
                            frames=list(generate_mincore_frames(fig,
                                                                mincore_dat,
                                                                animation_speed)), 
                            interval=10)
    if 1:
        writer = anm.FFMpegWriter(fps=15, metadata={"artist": "Me"},
                                  bitrate=1800)
        ani.save("mincore.mp4")
    elif 0:
...
def animate_mincore(mincore_dat, animation_speed):
    writer = anm.FFMpegWriter(fps=15, metadata={"artist": "Me"}, bitrate=1800)
    fig = plt.figure()
    with writer.saving(fig, "mincore.mp4", dpi=100):
...
def main():
    ...
    return animate_mincore(micore_dat, 0.02)
