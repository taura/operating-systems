...
\begin{frame}
  ...
  \item<3-> \ao{\tt int $e$ = ftruncate({\it fd}, {\it len});}
    \begin{itemize}
...
handle_page_fault(void * a, access_type rw) {
  ...
  if (@\ao{aはmmapでファイルと関連付けられた領域}@) {
    aの中身を@\ao{関連付けられたファイル}@から読み込み, pをそれで埋める;
  } else if (aへのアクセスは初めて) {
    pを0で埋める;
  } else {
...
%%%%%%%%%%%%%%%%% 
\begin{frame}[fragile]
  \frametitle{OS内のページフォルト処理 (再掲)}
\begin{lstlisting}
handle_page_fault(void * a, access_type rw) {
  そのプロセスのアドレス空間記述表を見る;
  if (aへのアクセスrwは非合法) segmentation fault発生;
  if (aはmmapでファイルと関連付けられた領域) {


      p = 空いている物理ページ;
      aの中身を関連付けらたファイルから読み込み, pをそれで埋める;
...
%%%%%%%%%%%%%%%%% 
\begin{frame}[fragile]
  \frametitle{OS内のページフォルト処理 (+キャッシュ)}
\begin{lstlisting}
handle_page_fault(void * a, access_type rw) {
  そのプロセスのアドレス空間記述表を見る;
  if (aへのアクセスrwは非合法) segmentation fault発生;
  if (aはmmapでファイルと関連付けられた領域) {
    @\ao{\tt if (aがすでにキャッシュに有る) p = 対応するキャッシュのアドレス;}@
    else {
      p = 空いている物理ページ;
      aの中身を関連付けらたファイルから読み込み, pをそれで埋める;
    }
    @\ao{if (aがwrite不可または{\tt MAP\_PRIVATE}指定された領域)}@
      @\ao{aをwrite不可に設定;}@
  } else {
    p = 空いている物理ページ;
    if (aへのアクセスは初めて) {
