...
%  \frametitle{futex (FUTEX\_WAIT), cond\_waitなどを呼ばずに待つことの問題}
  ...
  \item \ao{休眠待機 (blocking wait)}
    ...
    \end{itemize}
  \item \ao{頻忙待機 (busy wait, spin wait)}
    \begin{itemize}
...
%%%%%%%%%%%%%%%%% 
\begin{frame}
  \frametitle{スピンロック (頻忙待機で排他制御)}
  \begin{itemize}
  \item 頻忙待機による排他制御は特に\ao{スピンロック}と呼ばれて,
    短いクリティカルセクションを不可分に実行するのに使われる
  \item 効用はmutexと同様(違いは頻忙待機であること)
  \item Pthread API
    \begin{itemize}
    \item {\tt \#include <pthread.h>}
